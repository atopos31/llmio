...
export const BACKUP_ENCRYPTION = {
  TYPE: "llmio-webdav-backup-encrypted",
  VERSION: 1,
  KDF: "PBKDF2",
  CIPHER: "AES-GCM",
} as const;
...
export interface EncryptedBackupEnvelopeV1 {
  type: typeof BACKUP_ENCRYPTION.TYPE;
  v: typeof BACKUP_ENCRYPTION.VERSION;
  kdf: typeof BACKUP_ENCRYPTION.KDF;
  cipher: typeof BACKUP_ENCRYPTION.CIPHER;
  iter: number;
  salt: string;
  iv: string;
  ct: string;
...
/**
 * Attempt to detect and parse an encrypted backup envelope.
 *
 ...
 */
export function tryParseEncryptedBackupEnvelope(
  content: string
): EncryptedBackupEnvelopeV1 | null {
  try {
    const obj = JSON.parse(content) as Partial<EncryptedBackupEnvelopeV1>;

    if (
      obj &&
      obj.type === BACKUP_ENCRYPTION.TYPE &&
      obj.v === BACKUP_ENCRYPTION.VERSION &&
      obj.kdf === BACKUP_ENCRYPTION.KDF &&
      obj.cipher === BACKUP_ENCRYPTION.CIPHER &&
      typeof obj.iter === "number" &&
      typeof obj.salt === "string" &&
      typeof obj.iv === "string" &&
      typeof obj.ct === "string"
    ) {
      return obj as EncryptedBackupEnvelopeV1;
    }
...
/**
 * Check if the content is an encrypted backup.
 */
export function isEncryptedBackup(content: string): boolean {
  return tryParseEncryptedBackupEnvelope(content) !== null;
}
 ...
 */
export async function encryptBackupContent(params: {
  /**
  ...
  return {
    type: BACKUP_ENCRYPTION.TYPE,
    v: BACKUP_ENCRYPTION.VERSION,
    kdf: BACKUP_ENCRYPTION.KDF,
    cipher: BACKUP_ENCRYPTION.CIPHER,
    iter: iterations,
...
/**
 * Decrypt an encrypted backup envelope and return the plaintext JSON
 * backup string.
 *
 * Throws when:
 ...
 */
export async function decryptBackupEnvelope(params: {
